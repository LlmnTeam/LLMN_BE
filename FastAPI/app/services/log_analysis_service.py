...
from app.core.log_analysis_config import log_analysis_settings as las
...
async def validate_openai_api_key(api_key: str) -> bool:
    try:
        ...
        await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.API_VALIDATION_TOKENS
        )
        return True
...
async def generate_log_summary(content: str, api_key: str):     
    try:
        ...
        response_text = await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.LOG_SUMMARY_TOKENS
        )
        
        # 긴급 여부 체크 섹션 찾기
        urgency_start_index = response_text.find(las.URGENCY_CHECK_MARKER)
...
async def generate_performance_summary(content: str, api_key: str) -> str:
    try:
        ...
        return await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.PERFORMANCE_SUMMARY_TOKENS
        )
    except Exception as e:
...
async def generate_daily_summary(content: str, api_key: str) -> str:
    try:
        ...
        return await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.DAILY_SUMMARY_TOKENS
        )
    except Exception as e:
...
async def generate_trend_summary(content: str, api_key: str) -> str:
    try:
        ...
        return await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.TREND_SUMMARY_TOKENS
        )
    except Exception as e:
...
async def generate_recommend(content: str, api_key: str) -> str:
    try:
        ...
        return await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.RECOMMENDATION_TOKENS
        )
    except Exception as e:
...
async def generate_hourly_summary(content: str, api_key: str) -> str:
    try:
        ...
        return await _invoke_llm(
            prompt=prompt,
            api_key=api_key,
            max_tokens=las.HOURLY_SUMMARY_TOKENS
        )
    except Exception as e:
...
async def _invoke_llm(
    ...
    api_key: str, 
    max_tokens: int = 1000, 
    temperature: float = las.DEFAULT_TEMPERATURE,
    model: str = las.DEFAULT_MODEL
) -> str:
    try:
        # LLM 초기화
